...
def get_parser():
    parser = argparse.ArgumentParser(
        description='SelfGCN with Temporal Prediction')
    parser.add_argument(
        '--work-dir',
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help='weight for temporal prediction loss')
...
class Processor():
    """
    Processor for Skeleton-based Action Recognition with Temporal Prediction
    """
    ...
    def compute_prediction_loss(self, x_pred, x_future):
        """
        计算时序预测损失
        Args:
            x_pred: (N*M, C, T/2, V) 预测的未来特征
            x_future: (N*M, C, T/2, V) 真实的未来特征
        Returns:
            pred_loss: 预测损失
        """
        if self.arg.pred_loss_type == 'mse':
            ...
            x_pred_flat = x_pred.permute(0, 2, 3, 1).contiguous().view(-1, x_pred.size(1))
            x_future_flat = x_future.permute(0, 2, 3, 1).contiguous().view(-1, x_future.size(1))
    ...
    def train(self, epoch, save_model=False):
        ...
        # === 新增：动态权重策略 (基于你的log分析) ===
        # 建议在命令行设置 --pred-loss-weight 0.1 (因为加了Gap后Loss会变大)
        base_weight = self.arg.pred_loss_weight

        if epoch < 30:
            # 第一阶段 (0-29): 强力正则化，强迫模型学习时序逻辑
            current_weight = base_weight
        elif epoch < 50:
            # 第二阶段 (30-49): 权重减半，为LR衰减做准备，减少干扰
            current_weight = base_weight * 0.5
        else:
            # 第三阶段 (50+): 关闭辅助任务，全力冲刺分类准确率
            current_weight = 0.0

        # 记录权重变化到 TensorBoard方便观察
        self.train_writer.add_scalar('pred_loss_weight', current_weight, self.global_step)
        # ==========================================
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            # forward: 模型返回分类分数、预测特征、真实未来特征
            output, x_pred, x_future = self.model(data)
            ...
            pred_loss_value.append(pred_loss.data.item())
            ...
            self.train_writer.add_scalar('pred_loss', pred_loss.data.item(), self.global_step)
        ...
        self.print_log(
            '\tMean training loss: {:.4f} (cls: {:.4f}, pred: {:.4f}).  Mean training acc: {:.2f}%.'.format(
                np.mean(loss_value), np.mean(cls_loss_value), np.mean(pred_loss_value), np.mean(acc_value)*100))
